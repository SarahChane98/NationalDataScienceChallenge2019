...
#Load the VGG model
vgg_conv = keras.applications.VGG16(weights='imagenet', include_top=False, input_shape=(50, 50, 3))

tf_config = tf.ConfigProto()
tf_config.gpu_options.allow_growth = True
sess = tf.Session(config=tf_config)

NAME = "VGG-ADAM7-totaltrain-0.5-dropout-0.0001LR"
...
# model.add(keras.layers.Lambda(lambda x: tf.image.per_image_standardization(x)))
...
model.fit(train_images, train_labels, batch_size=100, epochs=35, initial_epoch=0, validation_split=0.30, shuffle=True,
          class_weight=class_weights, callbacks=[tensorboard, checkpointfunc, earlystopping])
